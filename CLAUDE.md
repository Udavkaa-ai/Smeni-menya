...
Когда бампить:
- Меняется shape данных в localStorage
- Меняется API так, что старый клиент сломается (404, неправильные эндпоинты)
- SW-стратегия меняется
- Подозрительные баги «у всех залип старый код»

Бамп — формат `'YYYY.MM.DD-rN'`, например `'2026.05.02-r1'`. Каждый бамп = принудительный reload у всех, кто хоть раз открывал сайт.
